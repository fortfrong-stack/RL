...
def train_enhanced_task(task_type, num_episodes=1000, save_model=True, model_path=None, save_stats=True, stats_path=None):
    """
    ...
    Args:
        ...
        model_path: Path to save the model (default: models/enhanced_dqn_task_{task_type}.pth)
        save_stats: Whether to save training statistics
        stats_path: Path to save statistics (default: stats/enhanced_stats_task_{task_type}.json)
    
    Returns:
        Trained DQNAgentWrapper and EnhancedTrainingStats object
    """
    ...
    if stats_path is None:
        stats_path = f"stats/enhanced_stats_task_{task_type}.json"
    ...
    for episode in tqdm(range(num_episodes), desc=f"Training Enhanced Task {task_type}", unit="episode"):
        ...
        if episode % 100 == 0:
            print(f"Enhanced Task {task_type}, Episode {episode}, Total Reward: {total_reward:.2f}, "
    ...
    if save_stats:
        ...
        plot_path = f"plots/enhanced_training_task_{task_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
...
def evaluate_enhanced_agent(agent, task_type, num_episodes=10, render=False):
    """
...
def load_enhanced_trained_agent(task_type, model_path=None):
    """
    Load a trained agent from a saved enhanced model file.
...
def train_all_enhanced_tasks(num_episodes=1000):
    """
    Train agents for all three enhanced tasks.
    
    Args:
        num_episodes: Number of episodes to train each agent
    """
    print("Starting training for all enhanced tasks...")
    
    for task_type in tqdm([1, 2, 3], desc="Overall Enhanced Training Progress", unit="task"):
        print(f"\nTraining for Enhanced Task {task_type}...")
        agent, stats = train_enhanced_task(task_type, num_episodes=num_episodes)
        
        # Evaluate the trained agent
        print(f"\nEvaluating Enhanced Task {task_type} agent...")
        evaluate_enhanced_agent(agent, task_type, num_episodes=5)
    
    print("\nTraining completed for all enhanced tasks!")